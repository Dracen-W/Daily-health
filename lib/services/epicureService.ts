import type { EpicurePairingInput, RecognizedIngredientInput, RecipePreferenceInput } from "../types/domain";
...
async function mcpRequest(
  ...
  const response = await fetch(baseUrl, {
    ...
    // signal: AbortSignal.timeout(6000) // Removed for simplicity if not supported in all environments
  });
...
async function initializeSession(baseUrl: string): Promise<string> {
  const { response, sessionId } = await mcpRequest(baseUrl, {
    jsonrpc: "2.0",
    id: "DH_" + Math.random().toString(36).slice(2, 10),
    method: "initialize",
    params: {
      protocolVersion: "2024-11-05",
      capabilities: {},
      clientInfo: { name: "daily-health", version: "2.0.0" }
    }
  });
