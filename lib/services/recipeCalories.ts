...
const kcalPer100g: Record<string, number> = {
  apple: 52, basil: 23, beef: 250, bell: 31, bread: 265, broccoli: 34, butter: 717, cabbage: 25, carrot: 41, cheese: 402, chicken: 165, cilantro: 23, corn: 86, cucumber: 15, egg: 143, fish: 206, flour: 364, garlic: 149, ginger: 80, honey: 304, milk: 42, mushroom: 22, noodle: 138, oil: 884, olive: 884, onion: 40, peanut: 567, pepper: 251, pork: 242, potato: 77, rice: 130, salmon: 208, sauce: 60, sesame: 573, shrimp: 99, soy: 53, spinach: 23, sugar: 387, tofu: 76, tomato: 18, tuna: 132, yogurt: 61
};

const defaultPortionGrams: Record<string, number> = {
  clove: 3, egg: 50, fillet: 150, handful: 30, leaf: 1, onion: 110, piece: 100, potato: 170, slice: 25, tomato: 120
};
...
function kcalForIngredientName(name: string) {
  ...
  const matchedKey = Object.keys(kcalPer100g).sort((a, b) => b.length - a.length).find(k => normalized.includes(k));
...
function defaultGramsForName(name: string) {
  const normalized = normalize(name);
  const matchedKey = Object.keys(defaultPortionGrams).sort((a, b) => b.length - a.length).find(k => normalized.includes(k));
...
function deterministicRecipeCalories(recipe: GeneratedRecipeInput): CalorieEstimate {
  ...
  return { estimatedCaloriesPerServing: Math.max(0, Math.round(totalCalories / Math.max(recipe.servings, 1))), source: "math" };
}
...
function findNumberByKey(value: unknown, keys: string[]): number | undefined {
  ...
  for (const key of keys) {
    const direct = asNumber((value as any)[key]);
...
function collectItemCalories(value: unknown): number[] {
  if (Array.isArray(value)) return value.flatMap(collectItemCalories);
  if (!isObject(value)) return [];
  const calories = findNumberByKey(value, ["calories", "kcal", "energyKcal", "totalKcal"]);
  const hasFoodName = ["ingredient", "name", "food", "item"].some((key) => typeof (value as any)[key] === "string");
...
function parseEpicureCalories(raw: unknown, servings: number): CalorieEstimate | null {
  const perServing = findNumberByKey(raw, ["estimatedCaloriesPerServing", "caloriesPerServing", "kcalPerServing", "perServingCalories", "perServingKcal"]);
  if (perServing != null) return { estimatedCaloriesPerServing: Math.round(perServing), source: "epicure" };
  const total = findNumberByKey(raw, ["totalCalories", "totalKcal", "calories", "kcal", "energyKcal"]);
  if (total != null) return { estimatedCaloriesPerServing: Math.round(total / Math.max(servings, 1)), source: "epicure" };
  const itemCalories = collectItemCalories(raw);
  if (itemCalories.length > 0) return { estimatedCaloriesPerServing: Math.round(itemCalories.reduce((a, b) => a + b, 0) / Math.max(servings, 1)), source: "epicure" };
  return null;
}

async function estimateWithEpicure(recipe: GeneratedRecipeInput, sourceIngredients: RecognizedIngredientInput[], pairings: EpicurePairingInput[]): Promise<CalorieEstimate | null> {
  const { listEpicureTools, callEpicureTool } = await import("./epicureService");
  const tools = await listEpicureTools();
  const toolName = tools.find((tool) => nutritionTools.some((pattern) => pattern.test(tool.name)))?.name;
  if (!toolName) return null;
  const raw = await callEpicureTool(toolName, {
    recipe: {
      title: recipe.translations.find((t) => t.locale === "en")?.title ?? recipe.translations[0]?.title,
      cuisineStyle: recipe.cuisineStyle,
      servings: recipe.servings,
      ingredients: recipe.ingredients.map((i) => ({ normalizedName: i.normalizedName, name: i.nameEn, amount: i.amount, optional: i.isOptional }))
    },
    sourceIngredients: sourceIngredients.map((i) => ({ name: i.normalizedName, displayName: i.displayNameEn, estimatedAmount: i.estimatedAmount })),
...
export async function applyRecipeCalorieEstimates(input: {
  ...
  for (const recipe of input.recipes) {
    ...
    try { estimate = await estimateWithEpicure(recipe, input.sourceIngredients, input.pairings); } catch (error) { }
    estimate ??= deterministicRecipeCalories(recipe);
    output.push({ ...recipe, estimatedCaloriesPerServing: estimate.estimatedCaloriesPerServing });
  }
  return output;
}
