        ...
        {scan ? (
          <View style={shared.panel}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <Text style={shared.sectionTitle}>{t("smart.result")}</Text>
              <View style={{ backgroundColor: scan.overallConfidence === "high" ? "#ECFDF5" : "#FFFBEB", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: scan.overallConfidence === "high" ? "#059669" : "#D97706" }}>
                <Text style={{ fontSize: 10, fontWeight: "700", color: scan.overallConfidence === "high" ? "#059669" : "#D97706" }}>{t("smart.confidence")}: {scan.overallConfidence}</Text>
              </View>
            </View>

            <Text style={[shared.helper, { marginBottom: 16 }]}>{locale === "zh-CN" ? scan.uncertaintyNoteZh : scan.uncertaintyNoteEn}</Text>

            <View style={{ gap: 12 }}>
              {ingredients.map((ingredient, index) => (
                <View key={`${ingredient.normalizedName}-${index}`} style={{ borderTopWidth: index ? 1 : 0, borderTopColor: colors.line, paddingTop: index ? 12 : 0 }}>
                  {editing ? (
                    <View style={{ gap: 8 }}>
                    ...
                    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                      <View style={{ flex: 1 }}>
                        <Text style={{ color: colors.text, fontWeight: "700", fontSize: 15 }}>{locale === "zh-CN" ? ingredient.displayNameZh : ingredient.displayNameEn}</Text>
                        <Text style={shared.helper}>{ingredient.estimatedAmount}</Text>
                        {ingredient.notes ? <Text style={[shared.helper, { fontSize: 11, fontStyle: "italic" }]}>{ingredient.notes}</Text> : null}
                      </View>
                      <View style={{ alignItems: "flex-end" }}>
                        <Text style={{ color: colors.leaf, fontWeight: "700" }}>{ingredient.estimatedCalories ?? 0} kcal</Text>
                      </View>
                    </View>
                  )}
                </View>
            ...
            <View style={{ borderTopWidth: 1, borderTopColor: colors.line, marginTop: 16, paddingTop: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <Text style={{ fontWeight: "700", color: colors.text }}>{t("smart.totalCalories")}</Text>
              <Text style={{ fontWeight: "800", color: colors.leaf, fontSize: 18 }}>
                {ingredients.reduce((sum, i) => sum + (i.estimatedCalories ?? 0), 0)} kcal
              </Text>
            </View>

            <View style={[shared.row, { marginTop: 20 }]}>
              <Pressable style={[shared.secondaryButton, shared.flex]} onPress={() => setEditing((value) => !value)}>
                <Text style={shared.secondaryButtonText}>{editing ? t("smart.doneEditing") : t("smart.edit")}</Text>
              </Pressable>
              <Pressable style={[shared.primaryButton, shared.flex, loading && { opacity: 0.55 }]} disabled={loading} onPress={() => void confirm()}>
                {loading ? <ActivityIndicator color="white" /> : <Text style={shared.primaryButtonText}>{scan.confirmedAt ? t("smart.confirmAgain") : t("smart.confirmIngredients")}</Text>}
              </Pressable>
            </View>

            {scan.confirmedAt ? (
              <Pressable style={[shared.primaryButton, { marginTop: 12, backgroundColor: colors.leafDark }]} onPress={() => router.push("/(tabs)/recipes")}>
                <Text style={shared.primaryButtonText}>{t("smart.generate")}</Text>
              </Pressable>
            ) : null}
          </View>
        ) : null}
