import { type CachedAuthStatus, localStatus, createProfileId } from "./statusShared";

export { localStatus, createProfileId };
...
function parse(value: string | null): CachedAuthStatus | null {
  ...
  try {
    ...
    return { version: 1, subscribed: Boolean(result.subscribed), localMirror: false, testMode: false, profileId: result.profileId, accessToken: typeof result.accessToken === "string" ? result.accessToken : null };
  } catch { return null; }
}
...
export async function writeCachedAuthStatus(status: CachedAuthStatus) {
  if (typeof localStorage !== "undefined") localStorage.setItem(cacheKey, JSON.stringify({ ...status, localMirror: false, testMode: false }));
}
