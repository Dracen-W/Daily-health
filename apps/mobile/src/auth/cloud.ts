...
import type { CachedAuthStatus } from "./statusShared";
...
function baseUrl() {
  ...
  if (Platform.OS === "web") {
    if (typeof window !== "undefined" && window.location) {
      ...
      if (!configured && origin && !origin.includes("localhost")) {
        return origin.endsWith("/") ? origin : `${origin}/`;
      }
    }
  }

  // On Android emulators, 10.0.2.2 is the alias for the host machine's localhost.
  // We prefer this over LAN IPs (like 192.168.x.x) because LAN IPs often change.
